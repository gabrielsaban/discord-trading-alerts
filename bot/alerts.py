...
class RsiAlert(AlertCondition):
    ...
    def __init__(self, symbol: str, oversold: float = 30, overbought: float = 70, cooldown_minutes: int = 240):
...
class MacdAlert(AlertCondition):
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if latest['MACD'] > latest['Signal'] and prev['MACD'] <= prev['Signal']:
            message = f"🟢 **MACD BULLISH CROSS**: {self.symbol} | Price: {price_str}"
        # Check for bearish crossover (MACD crosses below Signal)
        elif latest['MACD'] < latest['Signal'] and prev['MACD'] >= prev['Signal']:
            message = f"🔴 **MACD BEARISH CROSS**: {self.symbol} | Price: {price_str}"
...
class EmaCrossAlert(AlertCondition):
    ...
    def __init__(self, symbol: str, short: int = 9, long: int = 21, cooldown_minutes: int = 240):
        super().__init__(symbol, cooldown_minutes)
        self.short = short
        self.long = long
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if ema_df['Cross_Up'].iloc[-1]:
            message = f"🟢 **EMA BULLISH CROSS**: {self.symbol} EMA{self.short} crossed above EMA{self.long} | Price: {price_str}"
        # Check for bearish cross (short EMA crosses below long EMA)
        elif ema_df['Cross_Down'].iloc[-1]:
            message = f"🔴 **EMA BEARISH CROSS**: {self.symbol} EMA{self.short} crossed below EMA{self.long} | Price: {price_str}"
...
class BollingerBandAlert(AlertCondition):
    ...
    def __init__(self, symbol: str, cooldown_minutes: int = 240, squeeze_threshold: float = 0.05):
        super().__init__(symbol, cooldown_minutes)
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if latest_price > current['BBU'] and prev_close <= prev['BBU']:
            message = f"🟢 **UPPER BB BREAKOUT**: {self.symbol} Price broke above upper band | Price: {price_str}"
        
        # Lower band breakout (price crosses below lower band)
        elif latest_price < current['BBL'] and prev_close >= prev['BBL']:
            message = f"🔴 **LOWER BB BREAKOUT**: {self.symbol} Price broke below lower band | Price: {price_str}"
        
        # Band squeeze (bandwidth narrowing significantly)
        elif current['BandWidth'] < self.squeeze_threshold and current['BandWidth'] < prev['BandWidth']:
            message = f"🟡 **BOLLINGER SQUEEZE**: {self.symbol} Bands narrowing, potential breakout | Price: {price_str}"
...
class VolumeSpikeAlert(AlertCondition):
    ...
    def __init__(self, symbol: str, threshold: float = 2.5, cooldown_minutes: int = 240, z_score: bool = False):
        super().__init__(symbol, cooldown_minutes)
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if self.z_score:
            message = f"📊 **VOLUME Z-SCORE SPIKE**: {self.symbol} {direction} | Z-score: {latest['z_score']:.1f} | Price: {price_str}"
        else:
            message = f"📊 **VOLUME SPIKE**: {self.symbol} {direction} | {latest['volume_ratio']:.1f}x average | Price: {price_str}"
...
class AdxAlert(AlertCondition):
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if latest['Strength'] and not prev['Strength']:
            ...
            message = f"📏 **STRONG {direction} TREND**: {self.symbol} ADX: {latest['ADX']:.1f} | Price: {price_str}"
        
        # Trend direction change during strong trend
        elif latest['Strength'] and prev['Strength'] and latest['Trend'] != prev['Trend']:
            new_direction = "BULLISH 📈" if latest['Trend'] == 'Bullish' else "BEARISH 📉"
            message = f"🔄 **TREND REVERSAL to {new_direction}**: {self.symbol} ADX: {latest['ADX']:.1f} | Price: {price_str}"
...
class PatternAlert(AlertCondition):
    ...
    def check(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if self._is_hammer(candles.iloc[-1]):
            message = f"🔨 **HAMMER PATTERN**: {self.symbol} Potential reversal | Price: {price_str}"
        
        # Simple evening star (bullish-neutral-bearish sequence at top)
        elif self._is_evening_star(candles.iloc[-3:]):
            message = f"⭐ **EVENING STAR**: {self.symbol} Potential bearish reversal | Price: {price_str}"
        
        # Simple morning star (bearish-neutral-bullish sequence at bottom)
        elif self._is_morning_star(candles.iloc[-3:]):
            message = f"⭐ **MORNING STAR**: {self.symbol} Potential bullish reversal | Price: {price_str}"
        
        # Engulfing pattern
        elif self._is_engulfing(candles.iloc[-2:]):
            pattern_type = "BULLISH" if candles.iloc[-1]['close'] > candles.iloc[-1]['open'] else "BEARISH"
            message = f"🔄 **{pattern_type} ENGULFING**: {self.symbol} | Price: {price_str}"
    ...
    def _is_evening_star(self, candles: pd.DataFrame) -> bool:
        ...
        c2_small = abs(candles.iloc[1]['close'] - candles.iloc[1]['open']) < abs(candles.iloc[0]['close'] - candles.iloc[0]['open']) * 0.5
    ...
    def _is_engulfing(self, candles: pd.DataFrame) -> bool:
        ...
        if c1_bullish != c2_bullish and c2_body > c1_body:
            if c2_bullish:  # Bullish engulfing
                return candles.iloc[1]['open'] < candles.iloc[0]['close'] and candles.iloc[1]['close'] > candles.iloc[0]['open']
...
class AlertManager:
    ...
    def remove_alert(self, symbol: str, alert_type: type):
        ...
        self.alerts[symbol] = [alert for alert in self.alerts[symbol] 
                             if not isinstance(alert, alert_type)]
