...
class MarketDataCache:
    ...
    def get(self, symbol: str, interval: str) -> Optional[pd.DataFrame]:
        ...
        if key in self.cache:
            ...
            max_age = self.max_age_seconds.get(interval, 300)  # Default to 5 minutes if unknown
            ...
            if age < max_age:
                ...
                logger.debug(f"Cache expired for {symbol} ({interval}), age: {age:.1f}s > {max_age}s")
    ...
    def cleanup(self) -> None:
        ...
        for (symbol, interval), (timestamp, _) in self.cache.items():
            ...
            max_age = self.max_age_seconds.get(interval, 300) * 5  # Keep for 5x max_age before cleanup
        ...
        if before_count > after_count:
            logger.info(f"Cache cleanup: removed {before_count - after_count} stale entries")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "memory_usage_mb": sum(df.memory_usage().sum() for _, (_, df) in self.cache.items()) / (1024 * 1024)
        }
