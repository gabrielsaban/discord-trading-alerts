...
class TestIntegration:
    ...
    def test_alert_flow_from_indicator_to_notification(self, mock_fetch_data, sample_ohlcv_data):
    ...
    def test_data_flow_from_binance_to_indicators(self, mock_get):
        ...
        klines_data = [
            ...
            [1627776000000, "40000", "41000", "39500", "40500", "100", 1627779599999, "4050000", 1000, "50", "2025000", "0"],
            [1627779600000, "40500", "42000", "40400", "41800", "120", 1627783199999, "5016000", 1200, "60", "2508000", "0"],
            # Add more candles
            [1627783200000, "41800", "42500", "41500", "42200", "90", 1627786799999, "3798000", 900, "45", "1899000", "0"],
            [1627786800000, "42200", "43000", "42000", "42800", "150", 1627790399999, "6420000", 1500, "75", "3210000", "0"],
            [1627790400000, "42800", "43500", "42700", "43200", "130", 1627793999999, "5616000", 1300, "65", "2808000", "0"],
        ]
        ...
        assert list(df.columns) == ['open', 'high', 'low', 'close', 'volume'], "DataFrame should have OHLCV columns"
        ...
        assert rsi is not None or macd is not None or bb is not None, "At least one indicator should be calculated"
    ...
    def test_user_specific_alerts(self, mock_fetch_data, sample_ohlcv_data):
        ...
        db.update_user_settings("user1", {"rsi_oversold": 35, "rsi_overbought": 65, "enabled_alerts": ["rsi"]})
        ...
        user1_rsi_alert = RsiAlert("BTCUSDT", oversold=35, overbought=65)  # More sensitive
        ...
        with patch('bot.alerts.calculate_rsi') as mock_rsi:
            ...
            mock_rsi.return_value = pd.Series([40, 37, 36, 34])  # Crosses below 35, but still above 30
            ...
            if user1_db_alerts:
                assert "RSI" in user1_db_alerts[0]['message'], "Alert should be about RSI"
